...
func NewHandler(k keeper.Keeper, pk cleank.Keeper) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		ctx = ctx.WithEventManager(sdk.NewEventManager())

		switch msg := msg.(type) {
		case types.MsgCreateValidator:
			return handleMsgCreateValidator(ctx, msg, k, pk)
...
func handleMsgCreateValidator(ctx sdk.Context, msg types.MsgCreateValidator, k keeper.Keeper, pk cleank.Keeper) (*sdk.Result, error) {
	...
	minStake := pk.GetMinStake(ctx)
	fmt.Println("GetMinStake: ",minStake)
	if msg.Value.Amount.LT(minStake.Amount) {
		return nil, ErrMinStake
	}
